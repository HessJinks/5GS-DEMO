...
**Ideal For:**
- 5G protocol testing
- Control plane procedure validation
- Network function integration testing
- Development and debugging
- Learning 5G architecture
- CI/CD testing of 5G applications

**Not Ideal For:**
- End-to-end internet connectivity testing
- Performance benchmarking with external traffic
...
5GS/
├── README.md                           # This file
├── kind-5gs.yaml                       # Kind cluster configuration
├── Dockerfile.open5gs                  # Open5GS build
├── Dockerfile.ueransim                 # UERANSIM build
...
│   │   ├── nrf.yaml                    # NRF deployment
│   │   ├── scp.yaml                    # SCP deployment
│   │   ├── ausf.yaml                   # AUSF deployment
│   │   ├── udm.yaml                    # UDM deployment
│   │   ├── udr.yaml                    # UDR deployment
│   │   ├── pcf.yaml                    # PCF deployment
│   │   ├── nssf.yaml                   # NSSF deployment
│   │   ├── bsf.yaml                    # BSF deployment
│   │   ├── configmap-amf.yaml          # AMF configuration
│   │   ├── configmap-smf.yaml          # SMF configuration
│   │   ├── configmap-upf.yaml          # UPF configuration
│   │   ├── configmap-bsf.yaml          # BSF configuration
│   │   ├── configmap-nssf.yaml         # NSSF configuration
│   │   ├── configmap-pcf.yaml          # PCF configuration
│   │   ├── amf.yaml                    # AMF deployment
│   │   ├── smf.yaml                    # SMF deployment
│   │   ├── upf.yaml                    # UPF deployment
│   │   └── webui.yaml                  # WebUI deployment
│   └── ueransim/
│       ├── configmap-gnb.yaml          # gNB configuration
│       ├── configmap-ue.yaml           # UE configuration
│       └── gnb-ue.yaml                 # Combined gNB+UE deployment
├── open5gs/                            # Open5GS source code
└── UERANSIM/                           # UERANSIM source code
```
...
### FQDN Advertise Addresses

All Open5GS network functions use FQDN format for service advertisement:
```yaml
sbi:
  server:
    - address: 0.0.0.0
...
This ensures proper service discovery in Kubernetes where pod IPs are ephemeral.

### PFCP and GTP-U Interfaces

Both SMF and UPF require advertise addresses for PFCP and GTP-U:
```yaml
pfcp:
  server:
    - address: 0.0.0.0
...
- This is required because UERANSIM simulates the radio interface
- UE and gNB communicate over localhost (127.0.0.1)
- They share the same network namespace

### UPF NAT Configuration

The UPF automatically configures NAT for UE traffic:
```bash
iptables -t nat -A POSTROUTING -s 10.45.0.0/16 ! -o ogstun -j MASQUERADE
```

This allows UE packets to be routed (within cluster limitations).
...
- Open5GS: AGPLv3
- UERANSIM: GPLv3
- This deployment configuration: MIT (or your choice)
...
For issues with:
- **Open5GS**: https://github.com/open5gs/open5gs/issues
- **UERANSIM**: https://github.com/aligungr/UERANSIM/issues
...
- UERANSIM project for the comprehensive 5G UE/RAN simulator
- Kubernetes and Kind communities
